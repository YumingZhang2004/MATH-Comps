...
// Noise (2D) + procedural sand albedo
float hash21(vec2 p)
{
...
// Phong Lighting with soft shadows and per-material spec control
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Raymarch
    int matID;
    vec3 hitP = raymarch(ro, rd, matID);
    ...
    // Material
    vec3 albedo;
    float specMul, shinMul;
    getMaterial(matID, hitP, albedo, specMul, shinMul);
    ...
    float sha = calcSoftshadow(hitP, lightDir, 0.02, 25.0, 0.12, 1);

    // Lighting
    vec3 lit = phongLighting(hitP, normal, viewDir, lightPos, lightColor, specMul, shinMul, sha);
    vec3 col = albedo * lit;
